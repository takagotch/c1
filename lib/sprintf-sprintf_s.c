...
//stdio.h wchar.h wprintf(), fwprintf(), swprintf()
//stdarg.h        vwprint(), vfwprint(), vswprint()
//scanf(), printf()
//
#include <stdio.h>
int sprintf(char* restrict dest, const char* restrict format, ... );
int sprintf_s(char* restrict dest, rsize_t n, const char* restrict format ... );

char buffer[80];
double x = 1234.5, y = 678.9, z = -753.1, a = x * y + z;

int output_len = sprintf(buffer, "For the input values %lf, %lf, and %lf,"
		"\nthe result was %lf.\n", x, y, z, a);
puts(buffer);
if(output_len >= 80)
{
